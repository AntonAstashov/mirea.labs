...
class HashTable {
    ...
    public static void main(String[] args) {
        ...
        // добавление
        ht.hashtabAdd(2,"two");
        ht.hashtabAdd(-2,"minus two");

        // поиск
        String s = ht.hashtabLookup(-5 );
        System.out.print(s);

        // удалиение
        boolean success = ht.hashtabDelete(-5 );
        if(success){
            System.out.println("-5 deleted from hash table");
        }else {
            System.out.println("-5 doesn't exist in hash table");
        }
    }
}
